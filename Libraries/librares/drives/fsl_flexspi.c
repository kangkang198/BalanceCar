...
AT_NONCACHEABLE_SECTION_INIT(uint32_t FLEXSPI_GetInstance(FLEXSPI_Type *base))
{
    uint32_t instance;
...
AT_NONCACHEABLE_SECTION_INIT(static uint32_t FLEXSPI_ConfigureDll(FLEXSPI_Type *base, flexspi_device_config_t *config))
{
...
AT_NONCACHEABLE_SECTION_INIT( status_t FLEXSPI_CheckAndClearError_noncache(FLEXSPI_Type *base, uint32_t status))
{
    ...
    if (status)
    {
        ...
        {
            while(1);   //由于断言并未在 RAM中，因此这里不能调用断言
            //assert(false);
        }
 ...
 */
AT_NONCACHEABLE_SECTION_INIT(void FLEXSPI_Init(FLEXSPI_Type *base, const flexspi_config_t *config))
{
    ...
    uint32_t index = ((uint32_t)s_flexspiClock[FLEXSPI_GetInstance(base)]) >> 8U;
    uint32_t shift = ((uint32_t)s_flexspiClock[FLEXSPI_GetInstance(base)]) & 0x1FU;
    volatile uint32_t *reg;
    reg = ((volatile uint32_t *)&CCM->CCGR0) + index;
    *reg = ((*reg) & ~(3U << shift)) | (((uint32_t)kCLOCK_ClockNeededRunWait) << shift);
#endif /* FSL_SDK_DISABLE_DRIVER_CLOCK_CONTROL */
 ...
 */
AT_NONCACHEABLE_SECTION_INIT(void FLEXSPI_GetDefaultConfig(flexspi_config_t *config))
{
 ...
 */
AT_NONCACHEABLE_SECTION_INIT(void FLEXSPI_SetFlashConfig(FLEXSPI_Type *base, flexspi_device_config_t *config, flexspi_port_t port))
{
    uint32_t configValue = 0;
...
*/
AT_NONCACHEABLE_SECTION_INIT(void FLEXSPI_UpdateLUT(FLEXSPI_Type *base, uint32_t index, const uint32_t *cmd, uint32_t count))
{
    assert(index < 64U);
 ...
 */
AT_NONCACHEABLE_SECTION_INIT(status_t FLEXSPI_WriteBlocking(FLEXSPI_Type *base, uint32_t *buffer, size_t size))
{
    ...
    while (size)
    {
        ...
        result = FLEXSPI_CheckAndClearError_noncache(base, status);
 ...
 * retval kStatus_FLEXSPI_IpCommandGrantTimeout IP command grant timeout detected
 */
AT_NONCACHEABLE_SECTION_INIT(status_t FLEXSPI_ReadBlocking(FLEXSPI_Type *base, uint32_t *buffer, size_t size))
{
    ...
    while (size)
    {
        ...
        else
        {
            ...
            {
                result = FLEXSPI_CheckAndClearError_noncache(base, base->INTR);
 ...
 * retval kStatus_FLEXSPI_IpCommandGrantTimeout IP command grant timeout detected
*/
AT_NONCACHEABLE_SECTION_INIT(status_t FLEXSPI_TransferBlocking(FLEXSPI_Type *base, flexspi_transfer_t *xfer)) 
{
    uint32_t configValue = 0;
    status_t result = kStatus_Success;
